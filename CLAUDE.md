...
**Garmin residential ingest agent (Session 5 — `scripts/garmin_agent/`, durable fix for the WAF block):** fetch on laptop (residential IP) → POST `/garmin/ingest`; Railway never contacts Garmin. Live-only lessons (import tests miss these):
- Resume garth session with `Garmin().login(tokenstore=<str>)` (>512 chars → internal `garth.loads`). `garth.loads(t); client.login()` re-runs SSO with empty creds → **401** from `sso.garmin.com`.
- garth blob (~4 KB) > Windows Credential Manager's ~2560-byte limit → `CredWrite` `WinError 1783`. Store garth token in gitignored file (`.garth_token`, `os.open(...,0o600)`); keep only small ingest token in keyring.
- `--login` (Garmin SSO) is datacenter/VPN-blocked (401) like the sync path — run from residential IP (egress guard now covers `--login`).
- NordVPN split tunnel: Windows venv `python.exe` redirects to the base interpreter → bypass BOTH `…\.venv\Scripts\python.exe` AND `C:\Python313\python.exe`, then **reconnect** VPN. Verify `check_egress` residential *while connected*.
- Recovery metrics are NOT in `get_stats` (=RHR+body battery only); sleep/HRV/readiness/status are separate endpoints — see `garmin_fetch.enrich_metric()`. `get_daily_stats` does NOT exist on garminconnect 0.2.25 — use `get_stats`.
- Railway deploys `master` → `/garmin/*` 404 until branch merged. `GARMIN_INGEST_ATHLETE_EMAIL` = existing `Athlete.email` (app login), NOT the Garmin email, else `/ingest` 400 "ingest athlete not found".
- `getpass` shows no keystrokes (by design); IDE-integrated terminals may not accept hidden input → use standalone PowerShell.
- Ingest **merges metrics by date** (not skip-if-exists): re-run shows `+N metrics` every time (updates, not dup rows) — by design, so late HRV/sleep land on re-sync.
